//
//  RecentQuestionTableViewController.swift
//  Polling
//
...
class RecentQuestionTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        self.databaseRef.child("user_profile").child(self.loggedInUser!.uid).observeSingleEventOfType(.Value) { (snapshot:FIRDataSnapshot) in
            ...
            self.databaseRef.child("polls/\(self.loggedInUser!.uid)").observeEventType(.ChildAdded, withBlock: { (snapshot:FIRDataSnapshot) in
                ...
                self.topicTableView.insertRowsAtIndexPaths([NSIndexPath(forRow:0,inSection:0)], withRowAnimation: UITableViewRowAnimation.Automatic)
            }){(error) in
    ...
    */
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let cell: PushRecentQuestionTableViewCell = tableView.dequeueReusableCellWithIdentifier("topicCell", forIndexPath: indexPath) as! PushRecentQuestionTableViewCell
